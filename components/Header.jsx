...
import { useSession, signIn, signOut } from "next-auth/react"; // signOut eklendi
...
export default function Header() {
  ...
  const [isProfileOpen, setIsProfileOpen] = useState(false); // Profil menüsü state'i
  ...
  const profileRef = useRef(null); // Profil referansı
  ...
  return (
    <>
      ...
      <header className="fixed top-3 left-1/2 transform -translate-x-1/2 z-[9999] w-[92%] max-w-[850px]">
        {/* Header Yüksekliği: py-1.5'ten py-2.5'e çıkarıldı */}
                ...
                <div className="relative" ref={profileRef}>
                  ...
                  >
                    <div className="w-8 h-8 rounded-full bg-gradient-to-tr from-cyan-500 to-blue-600 p-[1.5px] shadow-md shadow-cyan-900/30 group-hover:shadow-cyan-500/30 transition-shadow">
                      {/* Resim varsa resmi kullan yoksa ikonu */}
                      {session?.user?.image ? (
                  ...
                  {isProfileOpen && (
                    <div className="absolute top-full right-0 mt-4 w-72 rounded-2xl bg-[#0f263d]/95 backdrop-blur-xl border border-white/10 shadow-2xl overflow-hidden animate-in slide-in-from-top-2 duration-200 ring-1 ring-black/10">
                      ...
                      {/* Orta Kısım: Token Cüzdanı (Vurgulu) */}
                      <div className="p-3">
                        <div className="rounded-xl bg-gradient-to-br from-slate-800 to-slate-900 border border-white/10 p-3.5 flex flex-col gap-3 group relative overflow-hidden">
                            ...
                            <Link 
                              ...
                              className="mt-1 flex items-center justify-center gap-1.5 w-full py-2 rounded-lg bg-cyan-600 hover:bg-cyan-500 text-white text-[11px] font-bold transition-all shadow-lg shadow-cyan-900/20 active:scale-[0.98]"
                            >
                              <Icons.Plus />
                              Bakiye Yükle
                            </Link>
                      ...
                      {/* Alt Kısım: Menü Linkleri */}
                      <div className="p-1.5 pb-2">
                        <Link 
                          href="/profilim" 
                          onClick={() => setIsProfileOpen(false)}
                          className="flex items-center gap-3 px-3 py-2.5 rounded-lg text-xs font-medium text-slate-300 hover:text-white hover:bg-white/5 transition-colors"
                        >
                        ...
                        <button 
                          ...
                          className="w-full flex items-center gap-3 px-3 py-2.5 rounded-lg text-xs font-medium text-red-400 hover:text-red-300 hover:bg-red-500/10 transition-colors text-left"
                        >
