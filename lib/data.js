...
export async function getKararlarFromDB(filters = {}, cursor = undefined) {
  ...
  // ⚡ FTS vektörleri — ÇIPLAK kolon kullan.
  // ÖNEMLİ: Bunları ASLA COALESCE(...) / to_tsvector(...) içine sarma.
  // İfade içine sarılırsa Postgres GIN indekslerini (karartsv_main_gin /
  // karartsv_extra_gin) kullanamaz ve her satırda content'i baştan tokenize
  // ederek tam tablo taraması yapar. tsv_main / tsv_extra trigger veya
  // generated column ile her zaman dolu tutulmalı (bkz. migration .sql).
  const tsvMain  = `"tsv_main"`;
  const tsvExtra = `"tsv_extra"`;
  ...
  // --- içerikten hariç tut (virgülle çoklu) ---
  if (filters.qnot) {
