...
// searchParams undefined hatası için düzeltme uygulandı: { searchParams = {} }
export default async function KararlarPage({ searchParams = {} }) {
  ...
  return (
    // DEĞİŞİKLİK: bg-slate-900 (Daha açık, gri tonlu arka plan)
    <div className="relative min-h-screen bg-slate-900 text-slate-300 font-sans selection:bg-cyan-500/30 selection:text-white overflow-hidden">
      
      <div className="fixed inset-0 z-0 pointer-events-none">
        {/* Izgara Deseni daha belirgin */}
        <div className="absolute inset-0 bg-[linear-gradient(to_right,#ffffff08_1px,transparent_1px),linear-gradient(to_bottom,#ffffff08_1px,transparent_1px)] bg-[size:40px_40px]"></div>
        {/* Tepe Işığı daha güçlü */}
        <div className="absolute top-0 left-1/2 -translate-x-1/2 w-full h-[700px] bg-gradient-to-b from-slate-800/80 via-slate-900/20 to-transparent pointer-events-none"></div>
      </div>

      <header className="sticky top-0 z-50 w-full border-b border-slate-700/60 bg-slate-900/90 backdrop-blur-xl">
        <div className="max-w-screen-2xl mx-auto px-6 py-4 flex items-center justify-between">
          <div className="flex items-center gap-4">
             {/* Logo Kutusu daha açık */}
             <div className="relative w-10 h-10 bg-slate-800 rounded-xl border border-slate-700 flex items-center justify-center text-cyan-400 shadow-sm">
      ...
      <div className="relative z-10 max-w-screen-2xl mx-auto px-4 py-10 lg:grid lg:grid-cols-[280px_1fr] lg:gap-8 xl:gap-10">
        
        <aside className="hidden lg:block">
          <div className="lg:sticky lg:top-28 space-y-6">
            {/* Filtre Kutusu: Zemin daha açık (Slate-800) */}
          ...
          {hasSearch && (
            <section className="animate-in fade-in slide-in-from-bottom-4 duration-500">
               <div className="mb-8 flex items-center gap-4">
                  ...
                  <span className="text-cyan-500 font-mono text-xs font-bold tracking-widest bg-slate-800 px-4 py-1.5 rounded-full border border-slate-700">ARAMA SONUÇLARI</span>
                  <div className="h-px flex-1 bg-slate-700"></div>
               </div>
              <SearchResults
                items={searchResults}
