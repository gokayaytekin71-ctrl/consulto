...
// ── Entry kartı ──────────────────────────────────────────────────────────────
function EntryCard({ entry, color, onDelete, onToggleStar, onStartEdit, isEditing, editingValue, setEditingValue, onConfirmEdit, onCancelEdit }) {
  ...
  return (
    <div className={`group flex gap-3 ${entry.starred ? "relative" : ""}`}>
      ...
      {/* Kart */}
      <div className={`flex-1 min-w-0 rounded-2xl border bg-white px-4 py-3 shadow-sm transition-all group-hover:shadow-md ${entry.starred ? "border-amber-200 bg-amber-50/30" : "border-slate-100 group-hover:border-slate-200"}`}>
        {/* Üst satır: saat + düzenlendi + aksiyon butonları */}
        <div className="flex items-center justify-between gap-2 mb-2">
          <div className="flex items-center gap-2">
            <span className="text-[10px] font-bold text-slate-300">{time}</span>
            {entry.updatedAt && (
              <span className="text-[9px] font-bold text-slate-300 italic">· düzenlendi</span>
            )}
            {entry.starred && (
              <span className="rounded-full bg-amber-100 px-1.5 py-0.5 text-[9px] font-black text-amber-700">Önemli</span>
            )}
          </div>

          {/* Aksiyon butonları — hover'da çıkar */}
          {!isEditing && (
        ...
        {/* İçerik: düzenleme modu veya metin */}
        {isEditing ? (
          <div>
            <textarea
              ref={editRef}
          ...
          <>
            <p className={`text-[12.5px] font-medium leading-6 text-slate-800 whitespace-pre-wrap break-words ${!expanded && isLong ? "line-clamp-4" : ""}`}>
              {entry.text}
            </p>
            {isLong && (
...
function SectionsColumn({ vm }) {
  ...
  const totalChars   = activeNote?.sections.reduce((s, sec) =>
    s + sec.entries.reduce((ss, e) => ss + e.text.length, 0), 0) ?? 0;
              ...
              {isRenaming ? (
                ...
                >
                  ...
                  <div className="flex items-center gap-1 shrink-0">
                    {starCount > 0 && (
                      <span className="text-[9px] text-amber-400">★{starCount}</span>
                    )}
...
export default function NoteEditorPanel({ vm }) {
  ...
  // Yıldızlı ve normal girdiler ayrı
        ...
        <div className="flex flex-1 min-w-0 flex-col">
          ...
          <div ref={listRef} className="custom-scrollbar min-h-0 flex-1 overflow-y-auto px-4 py-4 space-y-4">
            {filteredEntries.length === 0 ? (
              ...
              <>
                ...
                {/* Tarihe göre gruplu normal notlar */}
          ...
          <div className="shrink-0 border-t border-slate-100 bg-white p-3">
            ...
            <p className="mt-1 px-1 text-[10px] font-bold text-slate-300">
              {newEntryText.length > 0 ? `${newEntryText.length} karakter · ${newEntryText.trim().split(/\s+/).filter(Boolean).length} kelime` : "Shift+Enter yeni satır"}
            </p>
          </div>
        </div>
      </div>
