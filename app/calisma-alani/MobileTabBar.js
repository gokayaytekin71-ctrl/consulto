...
// Dilekçe modunda: Sohbet | Dilekçe | Notlar
// Diğer modlarda : Sohbet | Kararlar | Mevzuat | Notlar
// =============================================================================
...
export default function MobileTabBar({ vm }) {
  ...
  const isPetition = workspaceMode === "petition_draft";

  const tabs = isPetition
    ? [
        {
          id: "chat",
          label: "Sohbet",
          ...
          return (
            <button
              ...
              className={`relative flex min-w-0 flex-1 flex-col items-center gap-1 px-1 py-2 transition-colors ${
                active
                  ? tab.id === "dilekce" ? "text-emerald-700" : "text-blue-700"
            ...
            >
              ...
              {active && (
                <span className={`absolute inset-x-3 top-0 h-0.5 rounded-full ${tab.id === "dilekce" ? "bg-emerald-600" : "bg-blue-600"}`} />
              )}
            </button>
          );
        })}
