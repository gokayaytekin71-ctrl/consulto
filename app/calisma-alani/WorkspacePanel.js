...
const ACCENTS = {
  chat:      { top: "from-blue-500 to-cyan-400",     dot: "bg-blue-500" },
  decisions: { top: "from-violet-500 to-purple-500", dot: "bg-violet-500" },
  statutes:  { top: "from-indigo-500 to-blue-500",   dot: "bg-indigo-500" },
  notes:     { top: "from-emerald-500 to-teal-400",  dot: "bg-emerald-500" },
};

export default function WorkspacePanel({ id, title, subtitle, children, setActivePanel, actions }) {
  const accent = ACCENTS[id] ?? ACCENTS.chat;
  ...
  return (
    <section className="flex h-full min-h-[260px] overflow-hidden rounded-3xl border border-slate-200/70 bg-white shadow-[0_2px_8px_rgba(0,0,0,0.05),0_8px_32px_-4px_rgba(0,0,0,0.07)] transition-shadow duration-200 hover:shadow-[0_4px_16px_rgba(0,0,0,0.06),0_16px_48px_-8px_rgba(0,0,0,0.10)]">
      <div className="flex min-w-0 flex-1 flex-col">
        {/* Accent stripe */}
        <div className={`h-[3px] w-full shrink-0 bg-gradient-to-r ${accent.top}`} />
